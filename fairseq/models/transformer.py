...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def calculate_compression_rate(self, src_lengths, prev_output_tokens):
        # print("src_lengths = ", src_lengths.size())
        # print(src_lengths)
        # print("prev_output_tokens = ", prev_output_tokens.size())
        # print(prev_output_tokens)
        # print("tgt_lengths = ", tgt_lengths.size())
        # print(tgt_lengths)
        # calcuate the length
        tgt_lengths = prev_output_tokens.ne(self.decoder.padding_idx).long().sum(-1)
        # calculate the rate
        compression_rate = tgt_lengths.float() / src_lengths.float()
    ...
    def forward(
        ...
        """
        # print("transformer_model input = ")
        # print("src tokens = ", src_tokens.size())
        # print(src_tokens)
        # print("src_lengths = ", src_lengths.size())
        # print(src_lengths)
        # print("prev_output_tokens = ", prev_output_tokens.size())
        ...
        # attention分布
        # print("decoder_out = ", decoder_out[1]['attn'][0].size())
        # print(decoder_out[1]['attn'][0].mean(dim=1))
        # print(decoder_out[1]['attn'][0].mean(dim=1).sum(dim=1))
        # exit()
        
        # 进行regularization
        if not self.args.l1_CR_regularization:
            compression_rate = None
        return decoder_out, compression_rate
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward_embedding(
        ...
        compression_rate: Optional[torch.Tensor] = None,
    ):
        ...
        # add compression rate embeding to token embedding.
        if compression_rate is not None:
            # print("compression rate = ", compression_rate.size())
            ...
            # scale the compression rate and acquire embedding. scale the compression rate to the whole sequence.
            cr_embedding_x = self.cr_embedding((compression_rate * self.args.CR_embedding_scale).long()).unsqueeze(-2)
            # print("cr_embedding_x = ", cr_embedding_x.size())
            # print(cr_embedding_x)
            x += cr_embedding_x

        # 加入compression rate的embedding。
    ...
    def forward(
        ...
        Args:
            ...
            compression_rate: Tensor: batch * src_length
        ...
        return self.forward_scriptable(
            src_tokens, src_lengths, return_all_hiddens, token_embeddings, compression_rate
        )
    ...
    def forward_scriptable(
        ...
        x, encoder_embedding = self.forward_embedding(src_tokens, token_embeddings, compression_rate)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        #  add compresion_rate to decoder embeddings.
        if compression_rate is not None:

            # print("compression embedding mean = ")
            # print(self.cr_embedding.weight.abs().mean(dim=1))
            # exit()
            ...
            # print("compression rate = ", compression_rate.size())
            # print(compression_rate)
            # # print((compression_rate * self.args.CR_embedding_scale).long())
            # print('prev_output_tokens = ', prev_output_tokens.size())
            # print(prev_output_tokens)
            # print("x = ", x.size())
            # print(x)
            if isinstance(compression_rate, float):
                compression_rate = torch.Tensor([compression_rate]).to(x.device).repeat(x.size(0)).float()

            # scale the compression rate and acquire embedding.
            cr_embedding_x = self.cr_embedding((compression_rate * self.args.CR_embedding_scale).long()).unsqueeze(-2)
            # print("cr_embedding_x = ", cr_embedding_x.size())
            # print(cr_embedding_x)
            x += cr_embedding_x
        ...
        for idx, layer in enumerate(self.layers):
            ...
            # if layer_attn is not None:
                # 8 8 23 78 head * batch * target_length * source_length
                # print("layer_attn = ", layer_attn.size())
                # print(layer_attn)
                # print("layer_attn_sum = ", layer_attn.sum(dim=2).size())
                # print(layer_attn.sum(dim=2))
                # exit()
